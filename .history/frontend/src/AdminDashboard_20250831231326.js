...
function GasoilDashboard() {
    ...
    }, [chronoHistory]); // Utilisez chronoHistory pour inclure toutes les données
    // Add this new useMemo function inside the GasoilDashboard component
const getMonthlyAttributionByMachine = useMemo(() => {
    return attributionsHistory.reduce((acc, curr) => {
        const monthYear = moment(curr.date).format('YYYY-MM');
        if (curr.machinePlate) {
            if (!acc[monthYear]) acc[monthYear] = {};
            if (!acc[monthYear][curr.machinePlate]) {
                acc[monthYear][curr.machinePlate] = 0;
            }
            acc[monthYear][curr.machinePlate] += curr.liters;
        }
        return acc;
    }, {});
}, [attributionsHistory]);
    ...
    <FaChartLine />
    {isSidebarOpen && <span>Grands Bilans</span>}
</li>
...
<Row className="g-4">
    ...
    <Card className="dashboard-chart-card card-glass-light">
        <Card.Body>
            <Card.Title>Top Machines - Consommation de Gasoil (mensuel)</Card.Title>
            <Plot
                data={[{
                    x: getMonthlyAttributionByMachine[filterMonth] ? Object.keys(getMonthlyAttributionByMachine[filterMonth]).sort((a, b) => getMonthlyAttributionByMachine[filterMonth][b] - getMonthlyAttributionByMachine[filterMonth][a]) : [],
                    y: getMonthlyAttributionByMachine[filterMonth] ? Object.keys(getMonthlyAttributionByMachine[filterMonth]).sort((a, b) => getMonthlyAttributionByMachine[filterMonth][b] - getMonthlyAttributionByMachine[filterMonth][a]).map(key => getMonthlyAttributionByMachine[filterMonth][key]) : [],
                    type: 'bar',
                    marker: { color: getColorsForMachines(getMonthlyAttributionByMachine[filterMonth] ? Object.keys(getMonthlyAttributionByMachine[filterMonth]) : []) },
                    hovertemplate: '<b>%{x}</b><br>Litres: %{y}<extra></extra>',
                }]}
                layout={{
                    autosize: true,
                    height: 300,
