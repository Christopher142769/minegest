...
function GasoilDashboard() {
    ...
    <Card className="dashboard-chart-card card-glass-light">
        <Card.Body>
            <Card.Title>Évolution du Volume de Sable (m³)</Card.Title>
            <Plot
                data={getDailySableData.reduce((acc, curr) => {
                    // Chaque élément de getDailySableData est un point de données
                    // Nous devons regrouper les données par machine
                    if (!acc[curr.name]) {
                        acc[curr.name] = {
                            x: [],
                            y: [],
                    ...
                    acc[curr.name].x.push(curr.date); // Assurez-vous que curr.date existe
                    acc[curr.name].y.push(curr.volumeSable);
                    acc[curr.name].z.push(acc.length - 1); // Utiliser l'index pour la position Z
                    return acc;
                }, []).map((machineTrace) => {
                    // Convertir l'objet en tableau pour le composant Plot
                    return machineTrace;
                })}
                layout={{
                    autosize: true,
