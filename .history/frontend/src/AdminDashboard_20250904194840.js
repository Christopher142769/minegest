...
const getDailyGasoilData = useMemo(() => {
    // Vérifie que filterDate est un objet Date valide et non null ou indéfini.
    ...
    // Le format de la date est crucial pour la comparaison.
    const formattedFilterDate = filterDate.toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' });

    const dailyData = chronoHistory.filter(item => {
        if (!item.timestamp) return false;
        try {
            const itemDate = new Date(item.timestamp).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit', year: 'numeric' });
