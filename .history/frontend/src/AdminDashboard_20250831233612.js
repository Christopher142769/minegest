...
const getMonthlyConsumptionByMachine = useMemo(() => {
    const aggregatedData = attributionsHistory.reduce((acc, curr) => {
        // Obtenez l'année et le mois (ex: '2025-08')
        const monthYear = moment(curr.date).format('YYYY-MM');

        if (curr.machinePlate && curr.liters) {
            // Initialise le mois s'il n'existe pas encore
            if (!acc[monthYear]) {
                acc[monthYear] = {};
            }
            // Initialise la machine pour le mois s'il n'existe pas encore
            if (!acc[monthYear][curr.machinePlate]) {
                acc[monthYear][curr.machinePlate] = 0;
            }
            // Aggrège le total des litres pour cette machine et ce mois
