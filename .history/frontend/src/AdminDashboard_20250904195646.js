...
const getDailyGasoilData = useMemo(() => {
    ...
    // Utilisez le tableau gasOilHistory qui contient les données d'attribution
    if (!gasOilHistory || gasOilHistory.length === 0) {
        return [];
    }
    ...
    const dailyData = gasOilHistory.filter(item => {
...
}, [gasOilHistory, filterDate]); // N'oubliez pas de mettre gasOilHistory en dépendance
    return (
